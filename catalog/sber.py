...
class SberQR(object):
    ...
    def status(self, order_id: str, partner_order_number: str, order_sum: str):
        ...
        # определяю, если оплачено, то вставляю запись об оплате
        stat = response.json().get('order_state')
        if stat == 'PAID':
            sm=float(order_sum)
            dt=datetime.now()
            # оплачено, надо вставить запись

            r = Payment(date=dt, summa=sm, status=False, zakaz_date=dt, 
                        zakaz_number='-', zakaz_content='-')
            r.save()
